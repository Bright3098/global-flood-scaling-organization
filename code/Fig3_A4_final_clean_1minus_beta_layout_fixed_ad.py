...
FIG_DIR = ROOT_DIR / "_figures_fig3_A4_final_clean_1minus_beta_layout_fixed_ad"
...
def plot_fingerprint_summary(ax, effect_sum):
    ...
    # Panel a: compact legend placed safely inside the upper-left corner.
    # This avoids clipping and reduces overlap with the central fingerprint.
    ax.legend(
        frameon=False,
        fontsize=6.5,
        loc="upper left",
...
def plot_scale_by_class(ax_r, ax_c, scale_df, drivers, title, legend_loc="best", legend_bbox=None):
    ...
    for off, d in zip(offsets, drivers):
        ...
        ax_r.plot(
            ...
            marker="o",
            markersize=4.5,
            label=DISPLAY_LABELS.get(d, d).replace("\\n", " ")
        )
        ax_r.errorbar(
            xpos,
            sub["R_median_plot"],
    ...
    # Legend placement is panel-specific.
    # Panel d uses bbox_to_anchor to avoid covering the human-activity curves.
    if legend_bbox is None:
        ax_c.legend(frameon=False, fontsize=7, loc=legend_loc)
    else:
        ax_c.legend(
            frameon=False,
            fontsize=7,
...
ax_a.text(-0.085, 1.040, "a", fontsize=11, fontweight="bold", transform=ax_a.transAxes, clip_on=False)
...
plot_scale_by_class(ax_c1, ax_c2, warming_scale, WARMING_ORDER, "Warming", legend_loc="upper right")
...
ax_d1.text(-0.260, 1.040, "d", fontsize=11, fontweight="bold", transform=ax_d1.transAxes, clip_on=False)
plot_scale_by_class(ax_d1, ax_d2, human_scale, HUMAN_ORDER, "Human activity", legend_loc="lower center", legend_bbox=(0.50, -0.35))
...
for ext in ["pdf", "png", "svg"]:
    out = FIG_DIR / f"Fig3_A4_final_clean_1minus_beta_layout_fixed_ad.{ext}"
    if ext == "png":
        fig.savefig(out, dpi=PNG_DPI, bbox_inches="tight", pad_inches=0.10)
    else:
